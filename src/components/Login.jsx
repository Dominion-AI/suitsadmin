...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Datos del formulario antes de enviar:", formData); // Registro de datos del formulario
    setIsLoading(true);
    setError("");
    ...
    try {
      ...
      if (!isValidJWT(access) || !isValidJWT(refresh)) {
        throw new Error('Formato de token inválido recibido');
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-blue-50 to-white flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-xl">
        <div className="bg-white py-8 px-10 shadow-2xl rounded-xl sm:px-12 border border-gray-100">
          <div className="mb-8">
            <h2 className="text-3xl font-bold text-gray-800 text-center">Bienvenido de nuevo</h2>
            <p className="text-gray-500 mt-2 text-center">Inicia sesión en tu cuenta</p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              ...
              <input
                ...
                value={formData.username}
                onChange={handleChange}
                className="pl-3 py-3 w-full border-gray-300 rounded-lg"
            ...
            <div className="pt-4">
              <button type="submit" disabled={isLoading} className="w-full py-4 bg-indigo-600 text-white rounded-lg">
        ...
        <div className="text-center mt-8 text-sm text-gray-500">
          <p>© 2025 SuitsAdmin. Todos los derechos reservados.</p>
        </div>
      </div>
    </div>
  );
